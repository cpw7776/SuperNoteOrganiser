...
3. **`DOCUMENTATION GATE (User-Facing):`** — from `.claude/agents/docs-auditor-agent.md`. Section 1 (Help Center), 2 (FAQ), 3 (Onboarding Tour), 4 (Legal) all resolve to ALL NO with documented reasons; the applicable section is **1.b — User-Facing Surfaces** covering `.env.example` and the Streamlit sidebar copy in `note_organiser/app.py`. **Line 1.c (`README.md`) is MANDATORY** — same rigor as the CHANGELOG line in the Context Docs gate: every run must produce either a real update or an explicit, signal-grounded skip reason. The repo is public on GitHub; the README is the front door. Section 5 (PRD/ADR/Architecture rewrite) is fully applicable.
